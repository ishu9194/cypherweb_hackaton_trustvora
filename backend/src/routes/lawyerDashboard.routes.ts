...
import { claimCase, deleteLawyerCase, getLawyerCases, getUnassignedCases, lawyerRespondDeletion, updateLawyerCaseStatus } from "../controllers/cases.controller.js";
...
export async function lawyerDashboardRoutes(fastify: FastifyInstance) {
  ...
  fastify.delete("/cases/:id", { onRequest: [fastify.authenticate] }, deleteLawyerCase);
  fastify.post("/cases/:id/deletion-response", { onRequest: [fastify.authenticate] }, lawyerRespondDeletion);
