...
export function LawyerReviewsPage() {
  const { data: reviews, isLoading, error, refetch } = useAsync(() => lawyerDashboardService.getReviews(), []);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">{isLoading ? "Loading reviews…" : `${reviewsList.length} client reviews.`}</p>
        </div>
