...
function buildIcsFile(appointment: Appointment): string {
  ...
  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Trustora//Booking//EN",
    "BEGIN:VEVENT",
    `UID:${appointment.id}@trustora.in`,
    `DTSTAMP:${fmt(new Date())}`,
    `DTSTART:${fmt(start)}`,
    `DTEND:${fmt(end)}`,
    `SUMMARY:Consultation with ${appointment.lawyerName}`,
    `DESCRIPTION:Trustora ${appointment.type} consultation.`,
    "END:VEVENT",
    "END:VCALENDAR",
  ].join("\r\n");
}
...
export function BookingSuccessPage() {
  ...
  const handleAddToCalendar = () => {
    downloadTextFile(`trustora-${appointment.id}.ics`, buildIcsFile(appointment));
    toast.success("Calendar event downloaded — open it to add to your calendar");
  };

  const handleDownloadReceipt = () => {
    const content = [
      "Trustora — Booking Receipt",
      "================================",
    ...
    downloadTextFile(`trustora-receipt-${appointment.id}.txt`, content);
